...
- 対象を限定した判断基準・手順・実測記録は skill（`.claude/skills/<name>/SKILL.md`）へ置く。`.claude/rules/` は使わない: path-scoped rule が読み込まれるのは Read tool が一致するファイルを読んだときだけで、bash で読むと発火せずエラーも出ない（Claude Code 2.1.275 の docs で確認）
- skill の本文は一度ロードされると以降のターンに残り続ける。判断に使う基準だけを置き、公式 docs にある説明と長い検証記録は書かない（後者は `references/` へ分ける）
