...
- 対象を限定した判断基準・手順・実測記録は skill（`.claude/skills/<name>/SKILL.md`）へ置く。`.claude/rules/` は使わない: path-scoped rule が読み込まれるのは Read tool が一致するファイルを読んだときだけで、bash で読むと発火せずエラーも出ない
