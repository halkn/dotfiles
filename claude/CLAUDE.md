...
- コードコメントに置くのは「なぜ今この実装なのか」だけで、1 行に収める。commit 前に、追加したコメントを 1 つずつ次に照らして消す: ツールの公式ドキュメントにある説明・言語やツールの既定動作から自明なこと・What の言い換え・変更の経緯（「旧実装では」「〜を削除した」）・タスク ID / issue 番号の参照・根拠を伴わない Why（「パフォーマンスのため」等）・バージョンを伴う調査メモ（skill へ移す）。コメントは検証されないまま残りコードが変わっても追随しないので、迷ったら書かず commit message か設計ドキュメントに置く
