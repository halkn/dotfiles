...
OS パッケージ（zsh, unzip, bubblewrap, socat 等）は `[bootstrap.packages]` に `"apt:<pkg>"` 形式で宣言され、Linux では apt で不足分のみインストール、macOS では自動スキップされます。ログインシェルは `[bootstrap.user]` の `login_shell = "/bin/zsh"` から冪等に適用されます（`/etc/shells` 登録 + `chsh`）。
