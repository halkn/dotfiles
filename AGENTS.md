...
ターミナル多重化、`.config/mise/` はツール定義（mise で全 CLI ツールを管理）です。
...
- `just setup`: symlink 作成、mise tools install、Claude Code install、zsh plugin install を実行します。
...
- `just update`: mise tools 更新、`claude update`、zsh plugin を更新します。
