...
- `just setup`: symlink 作成、`mise install`、zsh plugin install を実行します。
...
- `just update`: mise 管理ツールと zsh plugin を更新します。
...
Shell ファイルと Lua ファイルは、`just fmt` が使う mise 管理の formatter で整形してください。
...
mise 管理ツールが未導入の場合は `just setup` を先に実行してください。
