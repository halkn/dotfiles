...
ターミナル多重化、`.config/mise/` はツール定義（CLI、LSP、formatter などを
[mise](https://mise.jdx.dev) で一元管理）です。
リポジトリ直下の `mise.toml` には project 用の task を定義します。
...
- `mise run setup`: symlink 作成、`mise install`、zsh plugin install を実行します。
...
- `mise run update`: mise 管理ツールと zsh plugin を更新します。

Agent は `mise run update` を自律実行せず、明示依頼がある場合だけ実行してください。
system package 更新が必要な場合は、mise task ではなくユーザーが個別に実行します。
...
Shell ファイルと Lua ファイルは、`mise run fmt` が使う mise 管理の formatter で整形してください。
...
Lua formatter は mise の `stylua` を正としてください。
diagnostics は mise の `lua-language-server --check` と
editor 内の `luals` を正とします。
`luals` の built-in formatter を再び主担当に戻さないでください。
Neovim 内で使う LSP server、efm backend tool、その他 CLI は
`.config/mise/config.toml` で管理し、`mise install` / `mise upgrade` で導入・更新します。

Neovim Lua を変更したときは、通常は `mise run fmt` で整形し、`mise run lint` で確認します。
`mise run lint-lua` は `stylua --check` と `lua-language-server --check` を確認し、
`mise run lint-nvim` は `nvim --headless -i NONE '+quitall'` で起動確認します。
mise 管理ツールが未導入の場合は `mise run setup` を先に実行してください。
...
- 通常: `mise run lint` を実行します。
- Neovim Lua: 変更後は `mise run fmt` と `mise run lint` を実行します。
- Shell: `zsh` 変更時は `mise run lint-zsh` または `mise run lint` の `zsh -n` 確認を通します。
