...
判断基準は `SKILL.md`。ここに置くのはこの環境で測った結果と、そこから外せない運用上の制約。

## docs を引く

記憶や過去の記録で代えず、書く前に一次情報を引く。

- `permissions` のパターン照合規則（`*` が空白を跨ぐ・末尾 `*` の語境界・複合コマンドの分割）: settings / permissions のページ
- sandbox の既定 allow と Troubleshooting（Seatbelt 下の Go 製 CLI・`unable to unlink old`）: sandboxing のページ
...
- 認証情報のパスは `filesystem.denyRead` ではなく `sandbox.credentials.files` に置く。適用される制限もパスのプレフィックス規則もスコープ間のマージも `denyRead` と共通だが、`credentials` 側は「これは認証情報だ」と宣言でき、`mode` が `deny` しか無いので広げる誤用も起きない。`filesystem.denyRead` は `~/` 全体のような広域ポリシーに使う
...
- Claude Code の自己保護が届くのは、ロード元のパス（cwd とその上位の `.claude/**`・`.mcp.json`、`~/.claude/**`）と、そこに張られた symlink の先だけ。`claude/hooks/*.sh`・`claude/statusline-command.sh` は Claude Code が実行するが symlink 先保護の対象外で、`mise.toml` と同様に Bash から書けた（v2.1.229・macOS で実測）。実体を repo 側に持つ設定・スクリプトは `filesystem.denyWrite` に実体パスで列挙する。副作用として、`claude/` 配下を変更する `git merge` / `git switch` は sandbox 内で `unable to unlink old` になる。`mise.toml` は日常的に編集するので deny に置かず、classifier に委ねる
- `~/.claude` は `filesystem.allowRead` に書かなくても読める（組み込みの allow に入る。v2.1.259・macOS で実測）。ただし届くのは `~/.claude` の実体までで、`~/.claude/skills/<name>` が別リポジトリへの symlink の場合（`git-flow`・`review-report` 等）は判定が解決後の実パスに移り、symlink 先が別途 `allowRead` に無いと Bash からは `Operation not permitted` になる（v2.1.267・macOS で実測）。symlink 先をリポジトリ単位で `allowRead` に足す
- `gh skill install --agent claude-code --scope user` は `~/.claude/skills/<name>` へインストールする際に symlink をそのまま辿る。`gh` は `sandbox.excludedCommands` で sandbox 外を走るため、`~/.claude/skills/<name>` が symlink のままだと `-f` で symlink 先リポジトリの実ファイルを直接上書きする（v2.1.267・gh 2.98.0 で実測）。symlink 運用から `gh skill install` の実体コピーへ切り替えるときは、**インストール前に symlink を消す**。symlink の削除自体は Claude Code の自己保護（`~/.claude/skills` が write の `denyWithinAllow` に入る）で Bash から拒否されるので、Claude Code の外のターミナルで行う
...
- `az login` のブラウザ認証は Apple Events がサンドボックスで塞がれるため失敗する。ログインはユーザーが Claude Code の外のターミナルで済ませ、Claude 側は `~/.azure` のトークンキャッシュを使う
...
- `herdr` は sandbox 内から実行すると `PermissionDenied`（`Os { code: 1 }`）になる。除外していないので、herdr を使う確認は Claude Code の外のターミナルで行う
