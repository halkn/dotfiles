...
- キャッシュは `~/.cache`（`XDG_CACHE_HOME`）を丸ごと allowRead / allowWrite にし、ツール別に列挙しない。列挙が増えるのはキャッシュだけで、`~/.local/share/{uv,mise}` は mise が管理ツールを全て配下に収めるため増えない
- 代わりに、`~/.cache` 配下で**閉じる側**を基準で決める。閉じるのは (1) 認証情報や取得済みのレスポンスを持つもの、(2) **ログインシェルやエディタが起動時に source / eval するもの**。(2) はサンドボックス外・classifier 外で実行されるので、書込を許すとサンドボックスを迂回した永続的なコード実行になる。このリポジトリでは `.zshrc` が `$XDG_CACHE_HOME/zsh/uv_completion.zsh` を source し、`compinit -C` で `.zcompdump` を検査なしで読み、fast-syntax-highlighting が `~/.cache/fsh/*.zsh` を読む。よって `~/.cache/{gh,zsh,bash,fsh,nvim}` を `denyWrite` に置く。`~/.cache/gh` は `credentials.files` の deny も併記し、read / write とも EPERM になることを確認済み（v2.1.259・macOS）。`gh` は `excludedCommands` でサンドボックス外を走るので、閉じても gh 自身は壊れない
- `~/.cache` へツールを足すときは、そのキャッシュがシェル・エディタから実行されるかを確認する。生成物を読むだけのキャッシュ（`uv` / `mise` / `rumdl` / `shuck`）は開けたままでよい
