...
- キャッシュは `~/.cache`（`XDG_CACHE_HOME`）を丸ごと allowRead / allowWrite にし、ツール別に列挙しない。列挙が増えるのはキャッシュだけで、`~/.local/share/{uv,mise}` は mise が管理ツールを全て配下に収めるため増えない。`~/.cache` 配下で閉じる必要があるのは `~/.cache/gh`（gh の API レスポンスキャッシュ）だけで、`credentials.files` の deny と `denyWrite` の両方に書いて read / write とも EPERM になることを確認済み（v2.1.259・macOS）。`gh` は `excludedCommands` でサンドボックス外を走るので、閉じても gh 自身は壊れない
