...
- `filesystem` の read 規則は**より具体的なパスが勝つ**。docs は `allowRead: ~/` の内側の `denyRead: ~/.env` が有効に残る例を明示している。よって `allowRead: ~/.config` の内側に `denyRead: ~/.config/gh` を置くのは有効な手。ただしこのリポジトリには v2.1.207 で「deny が実効しない」と実測した記録があり、docs と食い違う。`~/.config/gh` の deny は入れてあるが未検証なので、`claude/hooks/block-secret-read.sh` を backstop として残す。どちらが正しいか測ってから hook を外すこと（`sandbox.credentials.files` の deny は別で、`allowRead` が同じパスを再許可すると docs 上も無効化され `error` 扱いになる）
- sandbox から除外したコマンド（`excludedCommands`）には `denyRead` が効かない。`gh` は除外しているので、`~/.config/gh` を deny しても `gh` 自身の認証は壊れない。逆に `az` は sandbox 内で動くため `~/.azure` は allowRead が必要で、こちらは deny に落とせない
