...
- `tv` の設定は `.config/television/`（`config.toml` と `cable/*.toml`）。channel は workflow の都合ではなく単体のツールとして書き、行データを workflow が計算するもの（`wt` のフラグ、`repo` の配置規約）だけ zsh から tv に流す
- `tv update-channels` / `tv migrate-config` を実行しない。`~/.config` がこのリポジトリへの symlink なので checkout に書き込む
